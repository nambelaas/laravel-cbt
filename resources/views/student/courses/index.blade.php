    ...
    <section id="content" class="flex">
        ...
        <div id="menu-content" class="flex flex-col w-full pb-[30px]">
            ...
            <div class="course-list-container flex flex-col px-5 mt-[30px] gap-[30px]">
                ...
                @forelse ($myCourses as $index => $myCourse)
                    <div class="list-items flex flex-nowrap justify-between pr-10">
                        <div class="flex shrink-0 w-[300px]">
                            <div class="flex items-center gap-4">
                                ...
                                <div class="flex flex-col gap-[2px]">
                                    <p class="font-bold text-lg">{{ $myCourse->name }}</p>
                        ...
                        <div class="flex shrink-0 w-[150px] items-center justify-center">
                            <p class="font-semibold">
                                {{ \Carbon\Carbon::parse($myCourse->created_at)->format('F j, Y') }}</p>
                        </div>
                        @if ($myCourse->category->name == 'Data Analyst')
                            <div class="flex shrink-0 w-[170px] items-center justify-center">
                                <p class="p-[8px_16px] rounded-full bg-[#FFF2E6] font-bold text-sm text-[#F6770B]">
                                    {{ $myCourse->category->name }}</p>
                            </div>
                        @elseif ($myCourse->category->name == 'Programming')
                            <div class="flex shrink-0 w-[170px] items-center justify-center">
                                <p class="p-[8px_16px] rounded-full bg-[#EAE8FE] font-bold text-sm text-[#6436F1]">
                                    {{ $myCourse->category->name }}</p>
                            </div>
                        @elseif ($myCourse->category->name == 'Design')
                            <div class="flex shrink-0 w-[170px] items-center justify-center">
                        ...
                        <div class="flex shrink-0 w-[120px] items-center">
                            ...
                            @if ($course->nextQuestionId !== null)
                                <a href="{{ route('dashboard.learning.course', ['course' => $course->id, 'question' => $course->nextQuestionId]) }}"
                                    ...
                                    Test</a>
                            @else
                                <a href="{{ route('dashboard.learning.report.course', $course) }}"
                                    class="w-full h-[41px] p-[10px_20px] bg-indigo-800 rounded-full font-bold text-sm text-white transition-all duration-300 hover:shadow-[0_4px_15px_0_#6436F14D] text-center">Results</a>
                            @endif

                        </div>
                    </div>
                @empty
                    <span class="text-center font-semibold">No Course Provided</span>
                @endforelse
            </div>
        </div>
    </section>
